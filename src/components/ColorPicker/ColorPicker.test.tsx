...
describe('ColorPicker', () => {
	...
	it('selects initialColor if provided', () => {
		...
		expect(document.documentElement.style.getPropertyValue('--color-primary')).toBe(COLORS[2]);
	});
	...
	it('selects color on click and updates localStorage', () => {
		...
		expect(document.documentElement.style.getPropertyValue('--color-primary')).toBe(COLORS[1]);
	});
